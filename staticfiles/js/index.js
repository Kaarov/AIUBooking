...
daysli.forEach(li => {
    li.addEventListener("click", async () => {
        ...
        for (let i=0; i<bookingput.length; i++) {
            for (let j=0; j<bookingput[i].booking_time.length; j++) {
                list_booking_time.push(bookingput[i].booking_time[j]);
            }
        }
        // console.log(list_booking_time);

        let liTag = "";
        for (let i = 0; i < 18; i++) {
            if (list_booking_time) {
                if (list_booking_time[0]-1 === i) {
                    liTag += `<li class="booking-row"><span>${i+6} : 00</span><span>${i+7} : 00</span><a class="booked">booked</a></li>`;
